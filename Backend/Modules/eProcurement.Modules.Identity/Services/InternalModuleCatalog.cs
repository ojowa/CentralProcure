...
internal static class InternalModuleCatalog
{
    ...
    [
        ...
        new("create-requisition", "Create Requisition", "Requisitioning Departments", "Submit procurement requests with budget and justification metadata.", "Requisition Service", "Controlled initiation of procurement.", ActionSet("requisition.create"), RoleSet("RequisitioningOfficer", "DepartmentHead")),
        ...
        new("procurement-planning-committee", "Planning Committee Review", "Procurement Planning Committee", "Review APP assumptions across planning, finance, legal, and procurement members.", "Procurement Planning Service", "Section 21 planning committee visibility and pre-tender discipline.", ActionSet("planning_committee.view"), RoleSet("ProcurementOfficer", "ProcurementManager", "PlanningStatisticsOfficer", "FinancialUnitOfficer", "LegalReviewer")),
        new("budget-confirmation", "Budget Officer Workspace", "Financial Control", "Inspect APP funding readiness, validate budget line availability, and record budget confirmation decisions.", "Governance Service", "Distinct budget-gate control before APP approval.", ActionSet("planning_committee.view"), RoleSet("FinancialUnitOfficer", "AccountingOfficer", "Admin")),
        new("annual-procurement-plan", "Annual Procurement Plan (APP)", "Procurement Unit", "Create and maintain planning items aligned to statutory thresholds.", "Procurement Planning Service", "Legal planning compliance and budget discipline.", ActionSet("procurement_plan.manage"), RoleSet("ProcurementOfficer", "ProcurementManager", "SystemAdministrator", "Admin")),
        new("create-tender", "Create Tender", "Procurement Unit", "Draft tender packages, methods, and timelines before publication.", "Tender Management Service", "Method and threshold enforcement.", ActionSet("tender.manage"), RoleSet("ProcurementOfficer", "ProcurementManager")),
        new("publish-tender", "Publish Tender", "Procurement Unit", "Release approved tenders to the public portal at scheduled windows.", "Tender Management Service", "Controlled public disclosure.", ActionSet("tender.manage"), RoleSet("ProcurementOfficer", "ProcurementManager")),
        new("bid-opening-session", "Bid Opening Session", "Procurement Unit", "Open bids at scheduled deadlines with committee-level controls.", "Bid Opening Service", "Timed and committee-controlled opening.", ActionSet("bid_opening.manage", "bid_opening.view_detail"), RoleSet("ProcurementOfficer", "ProcurementManager", "SystemAdministrator", "Admin")),
        ...
        new("tender-review", "Tender Review", "Immigration Tender Board and Approvals", "Review committee outputs, clarifications, and exceptions.", "Approval Workflow Service", "Threshold-based approval governance.", ActionSet("approval.review"), RoleSet("TendersBoardMember", "TendersBoardSecretary")),
        new("approval-rejection", "Approval or Rejection", "Immigration Tender Board and Approvals", "Record board outcomes with mandatory rationale.", "Approval Workflow Service", "Non-repudiable approval decisions.", ActionSet("approval.decide"), RoleSet("TendersBoardMember", "TendersBoardSecretary")),
        new("high-value-tenders", "High-Value Tenders", "Accounting Officer", "Review tenders above delegated thresholds.", "Approval Workflow Service", "Accounting Officer authority for high-value spend.", ActionSet("high_value_tenders.review"), RoleSet("AccountingOfficer")),
