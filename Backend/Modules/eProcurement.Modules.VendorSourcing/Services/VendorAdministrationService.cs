...
public class VendorAdministrationService : IVendorAdministrationService
{
    ...
    public async Task<List<VendorApprovalSummary>> GetRegistrationsAsync(string? status, string? query, CancellationToken ct)
    {
        ...
        const string sql = @"
            ...
            WHERE (@p_status IS NULL OR LOWER(v.vendor_status) = LOWER(@p_status)) AND (@p_query IS NULL OR v.company_name ILIKE '%' || @p_query || '%' OR v.registration_number ILIKE '%' || @p_query || '%' OR v.tax_id ILIKE '%' || @p_query || '%' OR v.contact_person ILIKE '%' || @p_query || '%' OR COALESCE(v.phone_number, '') ILIKE '%' || @p_query || '%' OR v.email ILIKE '%' || @p_query || '%')
