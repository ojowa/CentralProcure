-- Migration 137: Fix vendor_sourcing.get_tenders — remove non-existent requisition_id column
-- The function referenced t.requisition_id which does not exist on vendor_sourcing.tenders.
-- This caused a 500 error on GET /api/Tender/open, breaking the keep-alive workflow
-- and putting Render to sleep (cold start timeout on dashboard load).
