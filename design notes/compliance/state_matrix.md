...
| `tenders_board_review` | Board-routed approval path | `procurement_workflow.evaluation_reports`, `procurement_workflow.workflow_instances`, `procurement_workflow.approval_thresholds` | `GET /api/tenders-board-approvals/queue`, `POST /api/tenders-board-approvals/{approve|reject|return}`, workflow action/runtime endpoints | `Tenders Board Approval`, `Tender Review`, `Approval or Rejection` | implemented | Runtime path exists through evaluation escalation, dedicated board decision endpoints, and threshold-aware routing to award publication or BPP |
| `accounting_officer_review` | CGIS approval path | `procurement_workflow.workflow_instances`, `procurement_workflow.approval_thresholds`, `procurement_workflow.requisitions` | `GET /api/workflow-runtime/cgis-queue`, `POST /api/cgis-approval/approve`, `reject`, `return`, `escalate` | `CGIS Approval`, `High-Value Tenders` | implemented | Direct executive approval flow is implemented with live queue views and contract-award creation on qualifying approval |
